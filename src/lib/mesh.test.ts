...
import { buildEdges, buildNodes } from "./mesh";

describe("buildNodes", () => {
  test("returns the requested number of points", () => {
    expect(buildNodes(70)).toHaveLength(70);
  });
  ...
  test("every point lies on the unit sphere", () => {
    ...
    for (const node of buildNodes(70)) {
...
describe("buildEdges", () => {
  test("links only pairs closer than the threshold", () => {
    ...
    for (const edge of buildEdges(nodes, 0.62)) {
      ...
      expect(distance).toBeLessThan(0.62);
    }
  });

  test("never links a node to itself and never repeats a pair", () => {
    const edges = buildEdges(buildNodes(70), 0.62);
  ...
  test("the default threshold produces a connected-looking mesh", () => {
    ...
    expect(edges.length).toBeGreaterThan(70);
    expect(edges.length).toBeLessThan(400);
  });

  test("a zero threshold produces no edges", () => {
    expect(buildEdges(buildNodes(70), 0)).toEqual([]);
  });
});
